...
*/
        ...
        public Dictionary<string, float> statisticalCharacteristics()
        {
            ...
            // Мера расхождения FIXME
            //Dictionary<float, float> D = new Dictionary<float, float>();
            float D = 0;
            for (int i = 0; i < size; i++)
            {
                D = 0;
               //D.Add((Erf((gaussDistribution_[i] - A * N) / Math.Sqrt(B * N)) + 1) / 2 , ((float)(i + 1) / (float)size));
              // D = Math.Max( Math.Abs((Erf((gaussDistribution_[i] - A * N) / Math.Sqrt(B * N)) + 1) / 2 - (float)i / (float)size) , D);
            }
